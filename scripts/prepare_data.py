...
def main():
    ...
    if not RAW_IMAGES_DIRNAME.exists():
        ...
        utils.extract_tar_file("/home/jun/Github/image-to-latex/data/formula_images.tar.gz")
    ...
    if not Path(cleaned_file).is_file():
        print("Cleaning data...")
        script = "/home/jun/Github/image-to-latex/scripts/find_and_replace.sh"
        subprocess.call(["/bin/sh", f"{str(script)}", "im2latex_formulas.norm.lst", cleaned_file])
